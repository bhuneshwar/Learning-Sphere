...
export const recordResourceDownload = async (downloadData) => {
  try {
    const response = await axios.post('/resource-downloads/record', downloadData);
...
export const getCourseDownloadStats = async (courseId) => {
  try {
    const response = await axios.get(`/resource-downloads/courses/${courseId}/stats`);
    return response.data;
  } catch (error) {
...
export const getUserDownloadHistory = async (page = 1, limit = 10) => {
  try {
    const response = await axios.get(`/resource-downloads/history?page=${page}&limit=${limit}`);
    return response.data;
  } catch (error) {
